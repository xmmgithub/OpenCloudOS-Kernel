...
static void pci_read_bridge_mmio_pref(struct pci_bus *child)
{
	...
	if ((mem_base_lo & PCI_PREF_RANGE_TYPE_MASK) == PCI_PREF_RANGE_TYPE_64) {
		...
		if (mem_base_hi <= mem_limit_hi) {
			...
			base64 |= (u64) mem_base_hi << 32;
			limit64 |= (u64) mem_limit_hi << 32;
			pr_info("jws[base>limit]:  base:%llu, limit:%llu\n", base64, limit64);
		}
	}
...
#ifdef CONFIG_FIX_OPTION_ROM
{
	u16 class;
	u32 bar_response;

	pci_bus_read_config_word(bus, devfn, PCI_CLASS_DEVICE, &class);
	class = class >> 8;
	if (class == PCI_BASE_CLASS_STORAGE) {
		pci_bus_write_config_dword(bus, devfn, 0x30, 0xffffffff);
		pci_bus_read_config_dword(bus, devfn, 0x30, &bar_response);
		pci_bus_write_config_dword(bus, devfn, 0x30, 0x0);
		}
}
#endif
