...
#define PARAMS_LEN  256
	...
	len = min((size_t)(PARAMS_LEN - 1), strlen(str));
	...
	while (p1 <= e) {
		...
		if (p2)
			*p2 = 0;
		if (!strcmp(p1, "blk")) {
			smmu_bypass_all_blkdev = true;
		} else {
			ret = sscanf(p1, "%hx:%hx", &vendor, &device);
			if (ret != 2)
				return -EINVAL;
...
static int arm_smmu_device_domain_type(struct device *dev, unsigned int *type)
{
	...
	if (smmu_bypass_all_blkdev &&
			(pdev->class >> 16) == PCI_BASE_CLASS_STORAGE) {
