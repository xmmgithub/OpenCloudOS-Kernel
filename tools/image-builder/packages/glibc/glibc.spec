...
Source2: SUPPORTED
Source3: locale-data.bin
Source4: locale-lib.bin
...
LANGUAGE=en_US.UTF-8 LC_ALL=en_US.UTF-8 make %{?_smp_mflags} -O -r

%install
LANGUAGE=en_US.UTF-8 LC_ALL=en_US.UTF-8 make -j1 install_root=%{buildroot} install -C build

mkdir -p %{buildroot}%{_cross_tmpfilesdir}
install -d %{buildroot}%{_cross_tmpfilesdir}
install -p -m 0644 %{S:1} %{buildroot}%{_cross_tmpfilesdir}/glibc.conf
mkdir -p %{buildroot}%{_cross_datadir}/locale/i18n
install -p -m 0644 %{S:2} %{buildroot}%{_cross_datadir}/locale/i18n/SUPPORTED
...
rm -rf %{buildroot}%{_cross_datadir}/locale/zh_CN
tar -xvf %{S:3} -C %{buildroot}%{_cross_datadir}
tar -xvf %{S:4} -C %{buildroot}%{_cross_libdir}
...
%{_cross_datadir}/i18n/charmaps/*
%{_cross_datadir}/i18n/locales/*
