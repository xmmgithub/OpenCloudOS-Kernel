...
static int mem_cgroup_meminfo_read(struct seq_file *m, void *v)
{
	...
	if (!memcg->meminfo_recursive) {
		mem_cache = memcg_page_state(memcg, MEMCG_CACHE);
		...
		mem_rss = memcg_page_state(memcg, MEMCG_RSS);
		mem_rss_huge = memcg_page_state(memcg, MEMCG_RSS_HUGE) * HPAGE_PMD_NR;
		mem_file_map = memcg_page_state(memcg, NR_FILE_MAPPED);
		mem_shmem = 0;
	} else {
		mem_cache = memcg_read_stat(memcg, MEMCG_CACHE);
		...
		mem_rss = memcg_read_stat(memcg, MEMCG_RSS);
		mem_rss_huge = memcg_read_stat(memcg, MEMCG_RSS_HUGE) * HPAGE_PMD_NR;
